...
public class AddTypeViewModel extends BaseViewModel {
     ...
     * @param name       类型名称
     */
    public Completable saveRecordType(RecordType recordType, int type, String imgName, String name,int mainTypeId) {
        if (recordType == null) {
            // 添加
            return mDataSource.addRecordType(type, imgName, name, mainTypeId);
        } else {
            // 修改
            RecordType updateType = new RecordType(recordType.id, name, imgName, recordType.type, recordType.ranking, recordType.mainTypeId);
