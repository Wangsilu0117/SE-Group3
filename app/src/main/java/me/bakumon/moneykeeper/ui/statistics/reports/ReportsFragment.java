...
public class ReportsFragment extends BaseFragment {
    ...
    protected void onInit(@Nullable Bundle savedInstanceState) {
        ...
        //mType = RecordType.TYPE_OUTLAY;           // 直接就是取值为二级支出
        mType = MainType.TYPE_OUTLAY;         // 直接改成一级支出，关键是Record.Dao中函数是筛 MainType的type
        initView();
    }
    ...
    private void initView() {
        ...
        mBinding.rgType.setOnCheckedChangeListener((group, checkedId) -> {
            if (checkedId == R.id.rb_outlay) {
                mType = MainType.TYPE_OUTLAY;
                getTypeSumMoney();
                getMonthSumMoney();
            } else if(checkedId == R.id.rb_income){
                mType = MainType.TYPE_INCOME;
                getTypeSumMoney();
                getMonthSumMoney();
            }else if(checkedId == R.id.rb_outlay_two){
    ...
    private void getTypeSumMoney() {
        mDisposable.add(mViewModel.getTypeSumMoneyOne(mYear, mMonth, mType)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    private void getMonthSumMoneyTwo() {
        mDisposable.add(mViewModel.getMonthSumMoney(mYear, mMonth)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(sumMoneyBean -> {
                            String outlay = "二级支出 ¥" + "0";
                            String income = "二级收入 ¥" + "0";
                            if (sumMoneyBean != null && sumMoneyBean.size() > 0) {
                                for (SumMoneyBean bean : sumMoneyBean) {
                                    if (bean.type == RecordType.TYPE_OUTLAY) {
                                        outlay = "二级支出 ¥" + BigDecimalUtil.fen2Yuan(bean.sumMoney);
                                    } else if (bean.type == RecordType.TYPE_INCOME) {
                                        income = "二级收入 ¥" + BigDecimalUtil.fen2Yuan(bean.sumMoney);
                                    }
                                }
                            }
                            mBinding.rbOutlayTwo.setText(outlay);
                            mBinding.rbIncomeTwo.setText(income);
                        },
                        throwable -> {
                            ToastUtils.show(R.string.toast_get_month_summary_fail);
                            Log.e(TAG, "获取该月汇总数据失败", throwable);
    ...
    private void getTypeSumMoneyTwo() {
        mDisposable.add(mViewModel.getTypeSumMoney(mYear, mMonth, mType)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(typeSumMoneyBeans -> {                  // 为什么获得了所有的mType对应的值？构成了列表？
                            setChartData(typeSumMoneyBeans);
                            mAdapter.setNewData(typeSumMoneyBeans);
                            if (typeSumMoneyBeans == null || typeSumMoneyBeans.size() < 1) {
                                mAdapter.setEmptyView(inflate(R.layout.layout_statistics_empty));
