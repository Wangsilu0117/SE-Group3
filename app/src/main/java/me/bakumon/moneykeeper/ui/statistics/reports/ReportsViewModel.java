...
public class ReportsViewModel extends BaseViewModel {
    ...
    // 直接改成：type为 MainType 中的一级取值
    public Flowable<List<TypeSumMoneyBean>> getTypeSumMoneyOne(int year, int month, int type) {
        Date dateFrom = DateUtils.getMonthStart(year, month);
        Date dateTo = DateUtils.getMonthEnd(year, month);
        return mDataSource.getTypeSumMoneyOne(dateFrom, dateTo, type);
    }
