...
public class HomeActivity extends BaseActivity implements StackCallback, EasyPermissions.PermissionCallbacks, EasyPermissions.RationaleCallbacks {
    ...
    private void initView() {
        ...
        //      wsl----------------------------------------------------------------------
        //点击item有反应了，但是不知道能不能实现跳转
        mAdapter.setOnItemChildClickListener((adapter, view, position) -> {
            Record bean = mAdapter.getData().get(position);
            String msg = "一级分类id："+bean.mainTypeId + "\n二级分类id："+bean.recordTypeId + "\n账户："+bean.accountId + "\n成员："+bean.memberId +
                         "\n商家："+bean.storeId + "\n项目"+bean.memberId;
    ...
    public void memberClick(View view) {
        Floo.navigation(this, Router.Url.URL_MEMBER)
                .start();
    }

    // lxy
    public void accountClick(View view) {
        Floo.navigation(this, Router.Url.URL_ACCOUNT).start();
    }
    ...
    //新加----------------------------------------------------------------
    private void  initMainTypes() {
        mDisposable.add(mViewModel.initMainTypes()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(()->{
                },
                        throwable -> {
                            if (throwable instanceof BackupFailException) {
                                ToastUtils.show(throwable.getMessage());
                                Log.e(TAG, "备份失败（初始化主类数据失败的时候）", throwable);
                            } else {
                                ToastUtils.show(R.string.toast_init_types_fail);
                                Log.e(TAG, "初始化主类数据失败", throwable);
                            }
                        })
        );
    }
    ...
    private void initAccounts(){
        mDisposable.add(mViewModel.initAccounts()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(()->{
                        },
                        throwable -> {
                            if (throwable instanceof BackupFailException){
                                ToastUtils.show(throwable.getMessage());
                                Log.e(TAG, "备份失败（初始化账户类型数据失败的时候）", throwable);
                            } else {
                                ToastUtils.show("初始化账户数据失败");
                                Log.e(TAG, "初始化账户数据失败", throwable);
                            }
                        }));
    }

    private void initTips(){
        mDisposable.add(mViewModel.initTips()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(()->{
                        },
                        throwable -> {
                            if (throwable instanceof BackupFailException){
                                ToastUtils.show(throwable.getMessage());
                                Log.e(TAG, "备份失败（初始化小贴士数据失败的时候）", throwable);
                            } else {
                                ToastUtils.show("初始化小贴士数据失败");
                                Log.e(TAG, "初始化小贴士数据失败", throwable);
                            }
                        }));
    }
    ...
    private void setTip(int i){
        mDisposable.add(mViewModel.getOneTip(i)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(tip -> {
                    AlertDialog.Builder builder = new android.support.v7.app.AlertDialog.Builder(this);
                    builder.setTitle("每天一个变美小贴士~");
                    builder.setMessage(tip.content);
                    builder.setPositiveButton("好的", null);
                    builder.show();
                        }, throwable -> {
                            ToastUtils.show(R.string.toast_get_types_fail);
                            Log.e(TAG, "获取贴士数据失败", throwable);}
                )
        );
    }
    ...
    private void initStores() {
        mDisposable.add(mViewModel.initStores()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(() -> {
                        },
    ...
    private void initProjects() {
        mDisposable.add(mViewModel.initProjects()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(() -> {
                        },
